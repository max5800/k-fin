"""Budget analysis agent — explains budget risks from deterministic context."""
...
from src.agents.prompts.budget_analysis import BUDGET_ANALYSIS_SYSTEM_PROMPT
...
BUDGET_ANALYSIS_MAX_TOKENS = 8000

budget_analysis_agent = Agent(
    make_anthropic_model(MODEL, prefer_prompted_output=True),
    output_type=AnalysisResult,
    system_prompt=BUDGET_ANALYSIS_SYSTEM_PROMPT,
    retries=2,
    model_settings=ModelSettings(max_tokens=BUDGET_ANALYSIS_MAX_TOKENS),
)
...
def run_budget_analysis(
    ...
    *,
    ...
    prompt = (
        f"## Budget-Analyse {period}\n\n"
        f"```json\n{json.dumps(safe_context, ensure_ascii=False, indent=2)}\n```\n\n"
        "Erstelle die Budget-Analyse gemäß dem Schema."
    )
    result = run_in_fresh_loop(budget_analysis_agent.run(prompt))
