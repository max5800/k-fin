...
def _make_engine_with_table(monkeypatch):
    ...
    Patches `main.create_engine` so endpoints that build their own engine
    via `settings.database_url` end up with this one.

    Also stubs out ``engine.dispose`` because the worker endpoints dispose
    after each call — we want to keep the in-memory data alive for the
    next assertion.
    """
